...
class FastScanner {
   ...
   */
  async findClaudeFiles(options: ScanOptions = {}): Promise<string[]> {
    const {
      path = process.cwd(),
      recursive = true,
      includeHidden = false,
   ...
   */
  async findSlashCommands(options: ScanOptions = {}): Promise<string[]> {
    const {
      path = process.cwd(),
      recursive = true,
      includeHidden = false,
...
if (import.meta.vitest != null) {
  ...
  describe('FastScanner', () => {
    ...
    test('should return version information', async () => {
      ...
      expect(version).toMatch(/fdir \d+\.\d+\.\d+/);
    });

    test('should find CLAUDE.md files', async () => {
      const files = await scanner.findClaudeFiles({
        path: process.cwd(),
        recursive: false,
      });
    ...
    test('should respect recursive option', async () => {
      const nonRecursive = await scanner.findClaudeFiles({
        path: process.cwd(),
        recursive: false,
      });

      const recursive = await scanner.findClaudeFiles({
        path: process.cwd(),
        recursive: true,
      });
    ...
    test('should find slash command files', async () => {
      ...
      const commands = await scanner.findSlashCommands({
        path: process.cwd(),
        recursive: true,
      });
